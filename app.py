...
def create_app():
    ...
    def process_queue():
        while True:
            ...
            response_data = {
                ...
                "build_number": BUILD_NUMBER  # Add build number
            }
            ...
            def wrapper(*args, **kwargs):
                ...
                if bypass_queue or 'webhook_url' not in data:
                    ...
                    return {
                        ...
                        "job_id": job_id,
                        "response": (
                            response[0] if response[2] == 200 else None
                        ),
                        "message": (
                            "success" if response[2] == 200 else response[0]
                        ),
                        "run_time": round(run_time, 3),
                        "queue_time": 0,
                    ...
                    if (
                        MAX_QUEUE_LENGTH > 0 
                        and task_queue.qsize() >= MAX_QUEUE_LENGTH
                    ):
                        message = (
                            f"MAX_QUEUE_LENGTH ({MAX_QUEUE_LENGTH}) reached"
                        )
                        return {
                            "code": 429,
                            "id": data.get("id"),
                    ...
                    # Define task function locally
                    def task_to_run():
                        return f(job_id=job_id, data=data, *args, **kwargs)
                        
                    task_queue.put((job_id, data, task_to_run, start_time))
                    
                    max_q_len_str = (
                        str(MAX_QUEUE_LENGTH) 
                        if MAX_QUEUE_LENGTH > 0 else "unlimited"
                    )
                    return {
                        "code": 202,
                        "id": data.get("id"),
                        ...
                        "pid": pid,
                        "queue_id": queue_id,
                        "max_queue_length": max_q_len_str,
                        "queue_length": task_queue.qsize(),
                        "build_number": BUILD_NUMBER  # Add build number
                    }, 202
            return wrapper
    ...
    from routes.v1.image.convert.image_to_video import (
        v1_image_convert_video_bp
    )
    ...
    from routes.v1.video.create_final_video import (
        v1_video_create_final_video_bp
    )
