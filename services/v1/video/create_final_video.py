...
def _send_webhook_notification(webhook_url: Optional[str], job_id: str, status: str,
                               data: Optional[Dict[str, Any]] = None,
                               error_message: Optional[str] = None) -> None:
    if not webhook_url:
        logger.debug(f"Job {job_id}: no webhook URL, skipping")
        return
...
def _create_segment_from_image(image_path: str, audio_path: str, out_path: str) -> None:
    ...
    logger.info(f"Creating static segment {out_path} (dur={dur}s)")
    cmd = [
        "ffmpeg","-y","-loop","1","-i",image_path,"-i",audio_path,
        "-c:v","libx264","-tune","stillimage","-c:a","aac","-b:a","192k",
        "-pix_fmt","yuv420p","-shortest","-t",str(dur),out_path
    ]
    try:
        _run_ffmpeg(cmd, f"Error creating segment {out_path}")
    except:
        logger.info(f"Fallback static for {out_path}")
        fb = ["ffmpeg","-y","-loop","1","-i",image_path,"-t",str(dur),
              "-c:v","libx264","-pix_fmt","yuv420p",out_path]
        _run_ffmpeg(fb, f"Fallback error {out_path}")

def _apply_zoom_effect(image_path: str, output_path: str,
                       zoom_type: str, zoom_speed: int,
                       audio_path: str) -> None:
    dur = _get_media_duration(audio_path)
    speed = max(1, min(20, zoom_speed)) / 10.0
    logger.info(f"Applying zoom '{zoom_type}' speed={zoom_speed}")
    if zoom_type == "Nenhum":
    ...
    cmd = ["ffmpeg","-y","-loop","1","-i",image_path,"-filter_complex",filt,
...
def _create_segment_with_audio(video_path: str, audio_path: str, out_path: str) -> None:
    logger.info(f"Combining video {video_path} + audio {audio_path}")
    cmd = ["ffmpeg","-y","-i",video_path,"-i",audio_path,
           "-c:v","copy","-c:a","aac","-map","0:v:0","-map","1:a:0","-shortest",out_path]
    _run_ffmpeg(cmd, f"Error combining {video_path}")

def _concatenate_with_transitions(files: List[str], out_path: str,
                                  trans_type: str, trans_dur: float) -> None:
    logger.info(f"Concatenating {len(files)} with transition={trans_type}")
    inputs, fc = [], []
    for i,f in enumerate(files):
        inputs += ["-i",f]
        fc.append(f"[{i}:v]setpts=PTS-STARTPTS[v{i}];[{i}:a]asetpts=PTS-STARTPTS[a{i}];")
    for i in range(len(files)-1):
        off = max(0,_get_media_duration(files[i]) - trans_dur)
        filt = f"xfade=transition=fade:duration={trans_dur}:offset={off}"
        fc.append(f"[v{i}][v{i+1}]{filt}[vt{i}];")
    ...
    try:
        _run_ffmpeg(cmd, f"Error concat trans {out_path}")
    except:
        logger.info(f"Fallback simple concat {out_path}")
        txt = tempfile.NamedTemporaryFile(mode="w",delete=False,suffix=".txt")
        for f in files:
            txt.write(f"file '{os.path.abspath(f)}'\n")
        txt.close()
        fb = ["ffmpeg","-y","-f","concat","-safe","0","-i",txt.name,
              "-c","copy",out_path]
        _run_ffmpeg(fb, f"Fallback concat {out_path}")
        os.unlink(txt.name)

def _add_background_music(video_path: str, music_url: str,
                          out_path: str, volume: int) -> None:
    tmp = tempfile.NamedTemporaryFile(suffix=".mp3",delete=False)
    tmp.close()
    _download_file(music_url, tmp.name)
    dur = _get_media_duration(video_path)
    vol = max(0,min(100,volume))/100.0
    logger.info(f"Adding BGM volume={volume}% → {out_path}")
    cmd = ["ffmpeg","-y","-i",video_path,"-stream_loop","-1","-i",tmp.name,
           "-filter_complex",
           f"[1:a]volume={vol},aloop=loop=-1:size=2e+09,atrim=end={dur}[bgm];[0:a][bgm]amix=inputs=2:duration=first[aout]",
           "-map","0:v","-map","[aout]","-c:v","copy","-c:a","aac","-shortest",out_path]
    _run_ffmpeg(cmd, f"Error BGM {out_path}")
    os.remove(tmp.name)
...
def create_final_video(
    ...
    logger.info(f"Job {job_id}: start pipeline ({len(scenes)} scenes)")
    ...
    try:
        ...
        for idx, scene in enumerate(scenes):
            ...
            out = os.path.join(job_dir, f"{DEFAULT_SEGMENT_PREFIX}{idx}.mp4")

            z = scene.get("options", {}).get("zoom", {})
            zt = z.get("type", opts["zoom"]["type"])
            zs = z.get("speed", opts["zoom"]["speed"])
            logger.debug(f"Zoom {zt}, speed {zs}")

            if zt != "Nenhum":
                tmpz = os.path.join(job_dir, f"zoom_{idx}.mp4")
                _apply_zoom_effect(img, tmpz, zt, zs, aud)
                _create_segment_with_audio(tmpz, aud, out)
            else:
                _create_segment_from_image(img, aud, out)

            logger.info(f"Job {job_id}: scene {idx} ready → {out}")
            segs.append(out)

        # 2) Concatenação
        tmp_final = os.path.join(job_dir, f"{job_id}_temp.mp4")
        ...
        if ttype == "Corte Seco":
            ...
            _run_ffmpeg(
                ["ffmpeg","-y","-f","concat","-safe","0","-i",txt,"-c","copy",tmp_final],
                f"Error simple concat job {job_id}"
            )
        else:
            _concatenate_with_transitions(segs, tmp_final, ttype, tdur)
        logger.info(f"Job {job_id}: concat done → {tmp_final}")

        # 3) Música
        bg = opts["background_music"]["url"]
        if bg:
            outbg = os.path.join(job_dir, f"{job_id}_bgm.mp4")
            _add_background_music(tmp_final, bg, outbg, opts["background_music"]["volume"])
            shutil.move(outbg, tmp_final)
            logger.info(f"Job {job_id}: BGM applied")
        ...
        if caps.get("enabled"):
            ...
            if txt_all:
                try:
                    capf = process_captioning(tmp_final, txt_all, caps.get("type","srt"), [], job_id)
                    if os.path.exists(capf):
                        shutil.move(capf, tmp_final)
        ...
        if ov.get("url"):
            ...
            _apply_overlay(tmp_final, ov["url"], outov, ov["position"], ov["opacity"])
            shutil.move(outov, tmp_final)
            logger.info(f"Job {job_id}: overlay applied")

        # 6) Upload
        key = f"final_videos/{job_id}_final.mp4"
        s3_url = upload_to_s3(tmp_final, key,
